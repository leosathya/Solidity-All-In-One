...
describe("Startt Testing Aave Flashloan", () => {
	const PROVIDER = "0xB53C1a33016B2DC2fF3653530bfF1848a515c8c5";
	...
	before(async () => {
		// deploy contracts UniswapSingleHop, AaveFlashloan
		...
		flashloanContract = await FlashloanContract.deploy(PROVIDER);
	...
	it("Request for Flashloan", async () => {
		...
		const tx = await flashloanContract
			.connect(accounts[0])
			.makeFlashloan(dai.address, flashloanAmount);
