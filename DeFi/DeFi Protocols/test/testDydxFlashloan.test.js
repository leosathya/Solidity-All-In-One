...
describe("Startt Testing Dydx Flashloan", () => {
	...
	before(async () => {
		// deploy contracts UniswapSingleHop, FlashloanDyDx
		// swap account eth for Dai, so that able to pay the primium
		// Flashloan Usdt from aave

		accounts = await ethers.getSigners();

		const FlashloanContract = await ethers.getContractFactory("FlashloanDydx");
	...
	it("Request for Flashloan", async () => {
		...
		const tx = await flashloanContract
			.connect(accounts[0])
			.initiateFlashloan(dai.address, flashloanAmount);
